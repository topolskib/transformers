...
class SegFormerModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_segformer_inputs_dict(config, input_ids, decoder_input_ids)
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = SegFormerModel(config=config).get_decoder().to(torch_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = SegFormerModel(config=config).to(torch_device).eval()
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = SegFormerEncoder.from_pretrained(tmpdirname).to(torch_device)
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = SegFormerDecoder.from_pretrained(tmpdirname).to(torch_device)
...
class SegFormerModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (SegFormerForConditionalGeneration,) if is_torch_available() else ()
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=SegFormerConfig)
    ...
    # SegFormerForSequenceClassification does not support inputs_embeds
    def test_inputs_embeds(self):
        config, inputs_dict = self.model_tester.prepare_config_and_inputs_for_common()

        for model_class in (SegFormerModel, SegFormerForConditionalGeneration, SegFormerForQuestionAnswering):
            model = model_class(config)
    ...
    def test_generate_fp16(self):
        ...
        model = SegFormerForConditionalGeneration(config).eval().to(torch_device)
...
class SegFormerStandaloneDecoderModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
    ...
    def create_and_check_decoder_model_past(
        ...
        model = SegFormerDecoder(config=config).to(torch_device).eval()
        # first forward pass
...
@require_torch
class SegFormerStandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (SegFormerDecoder, SegFormerForCausalLM) if is_torch_available() else ()
    all_generative_model_classes = (SegFormerForCausalLM,) if is_torch_available() else ()
    test_pruning = False
    is_encoder_decoder = False
    ...
    def setUp(
        ...
        self.model_tester = SegFormerStandaloneDecoderModelTester(self, is_training=False)
        self.config_tester = ConfigTester(self, config_class=SegFormerConfig)
