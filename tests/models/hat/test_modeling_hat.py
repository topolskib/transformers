...
class HATModelTester:
    ...
    def create_and_check_for_token_classification(
        ...
        model = HATForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = HATForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_question_answering(
        ...
        model = HATForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
...
@require_torch
class HATModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(self, config_class=HATConfig, hidden_size=37)
...
@require_torch
class HATModelIntegrationTest(TestCasePlus):
    @slow
    def test_inference_masked_lm(self):
        model = HATForMaskedLM.from_pretrained("kiddothe2b/hierarchical-transformer-base-4096")
        ...
        # hat = torch.hub.load('pytorch/fairseq', 'hat.base')
        # hat.eval()
        # expected_slice = hat.model.forward(input_ids)[0][:, :3, :3].detach()

        self.assertTrue(torch.allclose(output[:, :3, :3], expected_slice, atol=1e-4))

    @slow
    def test_inference_no_head(self):
        model = HATModel.from_pretrained("kiddothe2b/hierarchical-transformer-base-4096")
