# coding=utf-8
# Copyright 2023 The Microsoft Research Asia LayoutReader Team Authors, The Hugging Face Team.
#
...
from transformers import LayoutReaderConfig, is_torch_available
...
if is_torch_available():
    ...
    from transformers import (
        LayoutReaderForMaskedLM,
        LayoutReaderForQuestionAnswering,
        LayoutReaderForSequenceClassification,
        LayoutReaderForTokenClassification,
        LayoutReaderModel,
    )


class LayoutReaderModelTester:
    """You can also import this e.g from .test_modeling_layoutreader import LayoutReaderModelTester"""
    ...
    def create_and_check_model(
        ...
        model = LayoutReaderModel(config=config)
    ...
    def create_and_check_for_masked_lm(
        ...
        model = LayoutReaderForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = LayoutReaderForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
        result = model(
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = LayoutReaderForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, bbox, attention_mask=input_mask, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_question_answering(
        ...
        model = LayoutReaderForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
...
@require_torch
class LayoutReaderModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            LayoutReaderModel,
            LayoutReaderForMaskedLM,
    ...
    def setUp(self):
        self.model_tester = LayoutReaderModelTester(self)
        self.config_tester = ConfigTester(self, config_class=LayoutReaderConfig, hidden_size=37)
...
def prepare_layoutreader_batch_inputs():
    # Here we prepare a batch of 2 sequences to test a LayoutReader forward pass on:
    # fmt: off
...
@require_torch
class LayoutReaderModelIntegrationTest(unittest.TestCase):
    @slow
    def test_forward_pass_no_head(self):
        model = LayoutReaderModel.from_pretrained("microsoft/layoutreader-base").to(torch_device)

        input_ids, attention_mask, bbox, token_type_ids, labels = prepare_layoutreader_batch_inputs()
    ...
    def test_forward_pass_sequence_classification(self):
        ...
        model = LayoutReaderForSequenceClassification.from_pretrained("microsoft/layoutreader-base", num_labels=2).to(
            torch_device
        )

        input_ids, attention_mask, bbox, token_type_ids, _ = prepare_layoutreader_batch_inputs()
    ...
    def test_forward_pass_token_classification(self):
        ...
        model = LayoutReaderForTokenClassification.from_pretrained("microsoft/layoutreader-base", num_labels=13).to(
            torch_device
        )
    ...
    def test_forward_pass_question_answering(self):
        # initialize model with randomly initialized token classification head
        model = LayoutReaderForQuestionAnswering.from_pretrained("microsoft/layoutreader-base").to(torch_device)
