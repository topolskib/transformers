...
def prepare_layoutlm_batch_inputs():
    ...
    input_ids = torch.tensor([[  101,  1019,  1014,  1016,  1037, 12849,  4747,  1004, 14246,  2278,
          5439,  4524,  5002,  2930,  2193,  2930,  4341,  3208,  1005,  1055,
          2171,  2848, 11300,  3531,   102],
        [  101,  4070,  4034,  7020,  1024,  3058,  1015,  1013,  2861,  1013,
          6070, 19274,  2772,  6205, 27814, 16147, 16147,  4343,  2047, 10283,
         10969, 14389,  1012,  2338,   102]], device=torch_device)
    ...
    bbox = torch.tensor([[[   0,    0,    0,    0],
        ...
        [[   0,    0,    0,    0],
         ...
         [ 443,  653,  628,  672],
         [ 716,  301,  825,  317],
         [1000, 1000, 1000, 1000]]], device=torch_device)
    ...
    labels = torch.tensor([[-100,   10,   10,   10,    9,    1, -100,    7,    7, -100,    7,    7,
            4,    2,    5,    2,    8,    8, -100, -100,    5,    0,    3,    2,
         -100],
        [-100,   12,   12,   12, -100,   12,   10, -100, -100, -100, -100,   10,
           12,    9, -100, -100, -100,   10,   10,   10,    9,   12, -100,   10,
...
class LayoutLMModelIntegrationTest(unittest.TestCase):
    ...
    def default_tokenizer(self):
        return LayoutLMTokenizer.from_pretrained("microsoft/layoutlm-base-uncased")
    ...
    def test_forward_pass_no_head(self):
        ...
        self.assertTrue(torch.allclose(outputs.last_hidden_state[0, :3, :3], expected_slice, atol=TOLERANCE))

        # test the pooled output on [1, :3]
        expected_slice = torch.tensor([-0.6580, -0.0214,  0.8552], device=torch_device)

        self.assertTrue(torch.allclose(outputs.pooler_output[1, :3], expected_slice, atol=TOLERANCE))

    @slow
    def test_forward_pass_token_classification(self):
        model = LayoutLMForTokenClassification.from_pretrained("microsoft/layoutlm-base-uncased",
        num_labels=13).to(torch_device)
        ...
        expected_loss = torch.tensor(2.6169, device=torch_device)

        self.assertTrue(torch.allclose(outputs.loss, expected_loss, atol=TOLERANCE))

        # test the logits on [0, :3, :3]
        expected_slice = torch.tensor([[-0.0759, -0.0741, -0.2714],
                                        [-0.1292,  0.1305,  0.3655],
                                        [-0.4802,  0.4843,  0.5615]], device=torch_device)

        self.assertTrue(torch.allclose(outputs.logits[0, :3, :3], expected_slice, atol=TOLERANCE))