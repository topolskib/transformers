...
class BaseModelOutputWithSentenceAttentions(ModelOutput):
    ...
    Args:
        ...
        sentence_attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Sentence attentions weights after the attention softmax, used to compute the weighted average in the
...
class HATEncoder(nn.Module):
    ...
    def _tie_weights(self):
        """
        Tie the weights between sentence positional embeddings across all layers. If the `torchscript` flag is set in
        the configuration, can't handle parameter sharing so we are cloning the weights instead.
        """
...
@add_start_docstrings(
    """
    Hierarchical Attention Transformer (HAT) Model with a sequence classification/regression head on top (a linear
...
@add_start_docstrings(
    """
    Hierarchical Attention Transformer (HAT) Model with a multiple choice classification head on top (a linear layer on
    top of the pooled output and a softmax) e.g. for RocStories/SWAG tasks.
    """,
    HAT_START_DOCSTRING,
)
...
@add_start_docstrings(
    """
    Hierarchical Attention Transformer (HAT) Model with a token classification head on top (a linear layer on top of
...
@add_start_docstrings(
    """
    Hierarchical Attention Transformer (HAT) Model with a span classification head on top for extractive
    question-answering tasks like SQuAD (a linear layers on top of the hidden-states output to compute `span start
