    ...
    def process_multiple(self, html_strings, max_length=None, doc_stride=None, return_type="pt"):
        ...
        for i, sample_index in enumerate(sample_mapping):
            ...
            xpath_tags_in_span = [
                ...
                else self.xpath_tags_transfer(three_dict["string2xtag_seq"][sample_index][corr_word_id])
            ...
            xpath_subs_in_span = [
                self.PAD_XPATH_SUBS_SEQ
                if corr_word_id is None
                else self.xpath_subs_transfer(three_dict["string2xsubs_seq"][sample_index][corr_word_id])
                for corr_word_id in word_ids
            ]
        ...
        **kwargs
        ...
        if isinstance(html_strings, str):
            return self.process_multiple(
                [html_strings], max_length=max_length, doc_stride=doc_stride, return_type=return_type
            )
