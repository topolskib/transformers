...
MARKUPLM_START_DOCSTRING = r"""
    The MarkupLM model was proposed in ----- NOTHING!!!!!! -----
...
@add_start_docstrings("""MarkupLM Model with a `language modeling` head on top. """, MARKUPLM_START_DOCSTRING)
class MarkupLMForMaskedLM(MarkupLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.is_decoder:
            logger.warning(
                "If you want to use `MarkupLMForMaskedLM` make sure `config.is_decoder=False` for "
                "bi-directional self-attention."
            )

        self.markuplm = MarkupLMModel(config, add_pooling_layer=False)
        self.cls = MarkupLMOnlyMLMHead(config)
...
@add_start_docstrings(
    """
    MarkupLM Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a
    softmax) e.g. for RocStories/SWAG tasks.
    """,
    MARKUPLM_START_DOCSTRING,
)
class MarkupLMForMultipleChoice(MarkupLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    @add_start_docstrings_to_model_forward(
        MARKUPLM_INPUTS_DOCSTRING.format("batch_size, num_choices, sequence_length")
    )
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
...
class MarkupLMDecoderWrapper(MarkupLMPreTrainedModel):
    """
    ...
    def __init__(self, config):
        super().__init__(config)
        self.decoder = MarkupLMModel(config)

    def forward(self, *args, **kwargs):
        return self.decoder(*args, **kwargs)


class MarkupLMForCausalLM(MarkupLMPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        config = copy.deepcopy(config)
        config.is_decoder = True
        config.is_encoder_decoder = False
        self.model = MarkupLMDecoderWrapper(config)
    ...
    def forward(
        ...
        Args:
            input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`):
                ...
                provide it. Indices can be obtained using :class:`~transformers.MarkupLMTokenizer`. See
            ...
            past_key_values (:obj:`tuple(tuple(torch.FloatTensor))`, `optional`, returned when ``use_cache=True`` is passed or when ``config.use_cache=True``):
                ...
                model. Contains pre-computed hidden-states (key and values in the self-attention blocks and in the
                ...
                :obj:`(batch_size, 1)` instead of all ``decoder_input_ids`` of shape :obj:`(batch_size,
                sequence_length)`.
            labels (:obj:`torch.LongTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`):
        ...
        Example::
            >>> from transformers import MarkupLMTokenizer, MarkupLMForCausalLM
            >>> tokenizer = MarkupLMTokenizer.from_pretrained('microsoft/markuplm-large')
            >>> model = MarkupLMForCausalLM.from_pretrained('facebook/markuplm-large', add_cross_attention=False)
