...
class SwinPyramidPoolingModule(nn.ModuleList):
    """
    Args:
    Pyramid Pooling Module (PPM) used in PSPNet.
        pool_scales (tuple[int]): Pooling scales used in Pooling Pyramid
            Module.
        in_channels (int): Input channels. channels (int): Channels after modules, before conv_seg. align_corners
        (bool): align_corners argument of F.interpolate.
        ...
        for pool_scale in pool_scales:
            self.append(
                nn.Sequential(
                    ...
                    SwinConvModule(self.in_channels, self.channels, kernel_size=1),
                )
            )
...
class SwinFCNHead(nn.Module):
    ...
    [FCNNet](https://arxiv.org/abs/1411.4038>).
        config (SwinConfig): Configuration. in_channels kernel_size (int): The kernel size for convs in the head.
...
@add_start_docstrings(
    """
    Swin Transformer with a semantic segmentation head on top e.g. for ADE20k, CityScapes.
    """,
    SWIN_START_DOCSTRING,
)
class SwinForSemanticSegmentation(SwinPreTrainedModel):
    ...
    def forward(
        ...
        ```python
        >>> from transformers import SwinFeatureExtractor, SwinForSemanticSegmentation
        >>> from PIL import Image
        >>> import requests
        ...
        >>> feature_extractor = SwinFeatureExtractor.from_pretrained("microsoft/swin-base-finetuned-ade-640-640")
        >>> model = SwinForSemanticSegmentation.from_pretrained("microsoft/swin-base-finetuned-ade-640-640")
