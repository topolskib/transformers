...
Below, we illustrate how to use TAPEX for TableQA. As one can see, one can directly plug in the weights of TAPEX into a BART model.
...
# should print '['53', 'george clooney', 'brad pitt']'
```
...
benchmark for table fact checking (it achieves 84% accuracy).
...
# should print '0' (which means, refuted)
```